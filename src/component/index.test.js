...
describe('Grid', () => {
  ...
  it('allows to set width', () => {
    const noWidth = mount(
      <Grid itemHeight={250} itemWidth={250} items={items} height={500} />
    );
    expect(noWidth.find('.grid').props().style.width).toEqual('auto');

    const withWidth = mount(
      <Grid
        itemHeight={250}
        itemWidth={250}
        items={items}
  ...
  it('each child contains a wrapper', () => {
    const wrapper = getWrapper();
    expect(wrapper.find('.grid .grid-inner .item-wrapper').length).toEqual(
      wrapper.find('.grid .grid-inner .item-outer').length
    );
  });
  ...
  it('sets inner height based on amount of items', () => {
    ...
    expect(wrapper.find('.grid .grid-inner').props().style.height).toEqual(
      6250
    );
  });
  ...
  it('should contain styles for mobile', () => {
    const wrapper = getWrapper();
    expect(
      wrapper.find('.grid').props().style.WebkitOverflowScrolling
    ).toBeTruthy();
  });
